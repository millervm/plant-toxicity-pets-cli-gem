class PlantToxicity::Scraper
  ...
  def get_list(list)
    letter_details = @doc.search("div.view-content span.views-summary a").detect {|letters| letters.text == list.letter.upcase}
    if letter_details != nil
      ...
      Nokogiri::HTML(open(first_page)).search("div.views-field-title a").each do |plant|
        list.plants << PlantToxicity::Plant.new(plant.text, URL_BASE + plant.attribute("href").value)
      end
